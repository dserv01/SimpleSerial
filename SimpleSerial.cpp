...
int SimpleSerial::getKeyword(char *keywords[], uint8_t keyword_count) {
    nextToken();
    uint8_t i;
...
bool SimpleSerial::getUInt(uint16_t* ret) {
    (*ret)=0;
...
bool SimpleSerial::getInt(int *ret) {
...
//@TODO make it better without redundancy.
bool SimpleSerial::getDouble(double *ret){
    if(!nextToken()) return false;
    char sign = Serial.peek();
    if(sign=='+' || sign=='-') read();
    char x = read();
...
uint8_t SimpleSerial::getString(char* buffer, uint8_t keyword_count) {
    nextToken();
    uint8_t i=0;
    char x;
    do {
